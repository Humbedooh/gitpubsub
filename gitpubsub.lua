...
function checkGit(repo, name)
    ...
    for commit in data:gmatch("([^\r\n]+)") do
        ...
        if not id then
            ...
            ref = ref:gsub("^%(", ""):gsub("%)$", "")
            Xcommit = { repository="git",dirs_changed={name},project=name,hash=hash,id=id,author=author,email=email,subject=subject,log=subject,ref=ref,files={},revision=hash}
